...
// Inicialización de paginación con jQuery
jQuery(document).ready(function($) {
  const data = Array.from({length: 100}, (_, i) => ({
    id: i + 1,
    nombre: `Usuario ${i + 1}`,
    bono: `BONO00${String(i + 1).padStart(2, '0')}-${Math.floor(Math.random() * 9999)}`,
    estado: ['Entregado', 'Pendiente', 'En proceso'][Math.floor(Math.random() * 3)]
  }));
  ...
  $('#pagination-container').pagination({
    ...
    ulClassName: 'paginationjs-pages',
    callback: function(data, pagination) {
      console.log('Página cambiada:', pagination.pageNumber);
